...
if user_input:
    ...
    if not targets.empty:
        ...
        st.markdown(f"<h3 style='color: black; font-size: 14px;'>The selected target is: {selected_target_name}</h3>", unsafe_allow_html=True)
        ...
        if not data.empty:
            ...
            st.markdown(f"""
                <div style="color: black; text-align: center;">
                    Distributions of Lipinski Descriptors for {selected_target_name}
                </div>
            """, unsafe_allow_html=True)
