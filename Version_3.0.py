...
st.markdown('<p class="custom-text">Enter the protein name (e.g. EGFR) or ChEMBL id (e.g. ChEMBL2363049):</p>', unsafe_allow_html=True)
...
# Function to display a single row of GIFs
def display_row(original_smiles, mutants):
    # Create columns for the row
    col1, col2, col3 = st.columns(3)
    gif_counter = 1

    # Path to the original molecule GIF
    original_gif_path = f'{gif_counter}.gif'
    with col1:
        st.image(original_gif_path, caption=f"Original Molecule\nSMILES: {original_smiles}\npIC50: {predict_single_molecule(original_smiles, model)}")

    # Increment counter for mutants
    gif_counter += 1

    # Path to the mutant 1 GIF
    mutant_1_gif_path = f'{gif_counter}.gif'
    with col2:
        st.image(mutant_1_gif_path, caption=f"Mutant 1\nSMILES: {mutants[0]}\npIC50: {predict_single_molecule(mutants[0], model)}")

    # Increment counter for mutant 2 if it exists
    gif_counter += 1
    with col3:
        if len(mutants) > 1:
            # Path to the mutant 2 GIF
            mutant_2_gif_path = f'{gif_counter}.gif'
            st.image(mutant_2_gif_path, caption=f"Mutant 2\nSMILES: {mutants[1]}\npIC50: {predict_single_molecule(mutants[1], model)}")
        else:
            st.write("Mutant 2\nNo data")

# Iterate through each original molecule and its mutants
for original_smiles, mutants in best_mutants.items():
    display_row(original_smiles, mutants)
    st.write("---")
