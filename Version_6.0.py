...
#------ EQUIPO QUE DESARROLLÓ EL PROYECTO
st.markdown('<h3 style="color: black;">App developed by Biomini team</h2>', unsafe_allow_html=True)
...
This application is a powerful tool for bioactivity analysis and therapeutic molecule discovery powered by ML. The system allows for querying the database or loading local files containing molecular data, enabling seamless integration of information for subsequent analysis and processing
</p>
''', unsafe_allow_html=True)

st.image('WORKFLOW_AUTOSCREEN.jpeg')
...
st.markdown('<p class="custom-text", style= "font-weight: bold" >1. Enter the protein name (e.g. EGFR):</p>', unsafe_allow_html=True)
...
if user_input:
    ...
    if not targets.empty:
        ...
        st.markdown('<p class="custom-text", style= "font-weight: bold" >2. Targets retrived. Select target of interest:</p>', unsafe_allow_html=True)
        ...
        selected_target_id = st.selectbox("2.1. Select a target based on its target ChEMBL identificator (target_chembl_id):", target_names)
        ...
        if not data.empty:
            ...
            st.markdown(f"""
                <div style="color: black; text-align: center; font-weight: bold"
                    3. Bioactivity data with classification for {selected_target_name}
                </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown('<p class="custom-text", style= "font-weight: bold" >4. Distributions of Lipinski Descriptors of the molecules tested against the target protein:</p>', unsafe_allow_html=True)
           ...
           # Llamar a la función para obtener la imagen
            ...
            st.markdown('<p class="custom-text", style= "font-weight: bold" >5. Performance metrics of the tested models to predict the compound potency based on physicochemical and structural characteristics:</p>', unsafe_allow_html=True)
...
if not to_predict.empty:
    ...
    st.markdown('<p class="custom-text", style= "font-weight: bold" >7. Predictions:</p>', unsafe_allow_html=True)
...
st.markdown('<p class="custom-text", style= "font-weight: bold" >Original molecules and the first two more powerful mutants</p>', unsafe_allow_html=True)
...
def clean_smiles(smiles):
    # Implement cleaning for SMILES strings if needed.
...
def process_and_predict(input_df, smiles_col, id_col=None):
    """Predict potencies and return a DataFrame sorted by potency."""
    # Predict potencies
    input_df['Predicted_pIC50'] = input_df[smiles_col].apply(lambda x: predict_single_molecule(x, model))

    # Prepare the output DataFrame
    if id_col and id_col in input_df.columns:
        output_df = input_df[[id_col, smiles_col, 'Predicted_pIC50']]
    else:
        output_df = input_df[[smiles_col, 'Predicted_pIC50']]

    # Sort by predicted potency
    output_df.sort_values(by='Predicted_pIC50', ascending=False, inplace=True)

    return output_df

# Mensajes de introducción
st.markdown('<p class="custom-text", style= "font-weight: bold" >Recomendation</p>', unsafe_allow_html=True)
st.markdown('<p class="custom-text" >If you want to test your own molecule, we implemented this feature.</p>', unsafe_allow_html=True)
...
def clean_smiles_df(predicting):
  ...
  # Create DataFrames for Lipinski descriptors and ECFP4 fingerprints
  lipinski_df = pd.DataFrame(lipinski_data, columns=['MolWt', 'LogP', 'NumHDonors', 'NumHAcceptors'])
  ecfp4_df = pd.DataFrame(ecfp4_data, columns=[f'ECFP4_{i}' for i in range(2048)])

  # Concatenate the original dataframe with the Lipinski descriptors and ECFP4 fingerprints DataFrames
  all_info = pd.concat([predicting, lipinski_df, ecfp4_df], axis=1)
...
def predict_with_model(model, X):
    """Make predictions with the given model and feature matrix."""
    predictions = model.predict(X)
    return predictions



# to_predict = clean_smiles_df(predicting)
# predictions = predict_with_model(model, to_predict)
...
def process_and_predict(predicting):

    print(predicting)

    input_df = clean_smiles_df(predicting)
    predictions = predict_with_model(model, input_df)

    # Create a DataFrame with SMILES and predicted pIC50 values
    df = pd.DataFrame({
        'iD': predicting['zinc_id'],
        'SMILES': predicting['smiles'],
        'pIC50': predictions
    })

    # Sort the DataFrame by pIC50 in descending order
    output_df = df.sort_values(by='pIC50', ascending=False)
    return output_df
...
#     """Predict potencies and return a DataFrame sorted by potency."""
#     # Predict potencies
#     input_df['Predicted_pIC50'] = input_df[smiles_col].apply(lambda x: predict_with_model(x, model))
...
# Streamlit app
st.write("Upload a CSV file with SMILES strings")
...
#    smiles_col = st.text_input("SMILES column name", "smiles")
 #   id_col = st.text_input("Identifier column name (optional)", "")
    ...
    # Crear un botón usando HTML dentro de markdown
    if st.markdown('<button class="custom-button">Predict Potency</button>', unsafe_allow_html=True):
    
        # Process and predict
        result_df = process_and_predict(df)

        # Show results
        st.write("Predicted potencies:")
        st.write(result_df.head())

        # Provide download link for the results
        df_sorted = result_df.sort_values(by='pIC50', ascending=False)
        df_sorted = df_sorted.to_csv(index=False)
        ...
        button_html = f"""
            ...
            <a class="download-button" href="data:text/csv;base64,{csv_base64}" download="predicted_molecules.csv">Download Predictions</a>
        """
        # Mostrar el botón estilizado en Streamlit
        st.markdown(button_html, unsafe_allow_html=True)
        ...
        #     data=df_sorted,
        #     file_name="predicted_molecules.csv",
        #     mime="text/csv"
        # )
...
st.markdown("<p class='custom-text' >In the future, we plan to implement a security filter option based on Lipinski's Rule of Five, which helps in evaluating the drug-like properties of compounds. This rule assesses four key criteria related to molecular properties, such as molecular weight, lipophilicity, and the number of hydrogen bond donors and acceptors, to predict a compound's oral bioavailability and potential as a drug. We will use the RDKit library, a widely-used toolkit for cheminformatics, to calculate these properties and ensure that the compounds meet the necessary criteria for drug development.</p>", unsafe_allow_html=True)
