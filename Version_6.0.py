...
def clean_smiles(smiles):
    # Implementa la limpieza de SMILES si es necesario
    return pd.Series([smiles])

def predict_single_molecule(smiles, model):
    """Predecir el valor pIC50 para una sola molécula."""
    to_predict = clean_smiles(smiles)
    if to_predict.isnull().values.any():
        return None
...
def process_and_predict(input_df, smiles_col, id_col=None):
    """Predecir potencias y devolver un DataFrame ordenado por potencia."""
    # Predecir potencias
    ...
    # Ordenar por potencia predecida
...
st.text("")
# Opción de ingresar SMILES o CSV
st.markdown('<p class="custom-text" >Enter the smiles code:</p>', unsafe_allow_html=True)
user_input = st.text_area("", key='smiles_code', placeholder='Enter protein target here', height=30)
st.markdown('<p class="custom-text" >Or</p>', unsafe_allow_html=True)
uploaded_file = st.file_uploader("Choose a CSV file", type="csv")

# Si se ingresa un código SMILES directamente
if user_input:
    st.write("Processing single SMILES input...")
    prediction = predict_single_molecule(user_input, model)
    if prediction is not None:
        st.success(f"Predicted pIC50 value: {prediction}")
    else:
        st.error("Invalid SMILES input. Please check and try again.")
...
# Si se sube un archivo CSV
    ...
    if st.button('Predict Potency'):
        if smiles_col in df.columns:
            # Procesar y predecir
            result_df = process_and_predict(df, smiles_col, id_col if id_col else None)

            # Mostrar resultados
            st.write("Predicted potencies:")
            st.write(result_df.head())

            # Proveer enlace para descargar resultados
            ...
            st.error(f"Column '{smiles_col}' not found in the input CSV.")

# Si no se ingresa ni un SMILES ni se sube un archivo CSV
else:
    st.warning("Please enter a SMILES code or upload a CSV file.")
